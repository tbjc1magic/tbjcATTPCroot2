...
GETDecoder2:: ~GETDecoder2()
{
    if(fFrameInfoArray) delete fFrameInfoArray;
    if(fCoboFrameInfoArray) delete fCoboFrameInfoArray;
    if (fHeaderBase) delete fHeaderBase;
    if (fBasicFrameHeader) delete fBasicFrameHeader;
    if (fLayerHeader) delete fLayerHeader;
    if (fTopologyFrame) delete fTopologyFrame;
    if (fBasicFrame) delete fBasicFrame;
    if (fCoboFrame) delete fCoboFrame;
    if (fLayeredFrame) delete fLayeredFrame;
}
