...
class DataProcessor(object):
    ...
    def Process1(self,EID):
        ...
        for idx,q in enumerate(Q):
            image_ = np.zeros([300,600])
            image_[(q['PadPos'].values*pow(-1,idx/2+1)+150).astype(np.int), q['time'].values.astype(np.int)] =255
